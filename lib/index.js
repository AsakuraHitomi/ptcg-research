...
function apply(ctx, config) {
    ...
    if (/^\w+-\d+$/i.test(input)) {
      ...
      try {
        ...
        const response = await import_axios.default.get(
          `https://api.tcgdex.net/v2/${lang}/cards?name=${encodeURIComponent(input)}`
        );
        data = response.data;
        const filterOptions = {
  ...
  async function formatCardDetails(card, lang) {
    ...
    if (card.image) {
      ...
      while (retryCount < maxRetries && !imageAdded) {
        try {
          ...
          if (response.status === 200) {
            ...
            throw new Error(`图片状态码: ${response.status}`);
          }
        } catch (error) {
          retryCount++;
          if (retryCount < maxRetries) {
            ctx.logger.warn(`图片获取失败（${retryCount}/${maxRetries}），卡片：${card.name}`);
            await new Promise(resolve => setTimeout(resolve, 2000));
          } else {
            ctx.logger.error(`图片获取最终失败，卡片：${card.name}`, error);
          }
        }
      }
    }
